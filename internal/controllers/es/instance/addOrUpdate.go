...
func addOrUpdateK8sClient(instance *dal.Instance, authConfig *dal.AuthConfig) error {
	...
	return configs.AddK8sClient(instance, authConfig)
}

// removeK8sClient 移除k8s客户端
func removeK8sClient(instanceID uint) {
	configs.RemoveK8sClient(instanceID)
}
...
// addInstance 添加实例
func addInstance(repo *configs.InstanceRepository, req request.InstanceRequest) (int, error) {
	// 验证实例类型存在性
...
// updateInstance 更新实例
func updateInstance(repo *configs.InstanceRepository, req request.InstanceRequest) (int, error) {
	if req.ID == 0 {
...
func handleAuthConfig(repo *configs.AuthConfigRepository, instanceID uint, authConfig *request.AuthConfigRequest, isAdd bool) error {
	// 无认证配置时删除现有配置
